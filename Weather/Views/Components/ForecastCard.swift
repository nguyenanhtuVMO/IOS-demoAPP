...
struct ForecastCard: View {
    ...
    var isActive: Bool {
        if forecastPeriod == ForecastPeriod.hourly {
            let Hours = Calendar.current.isDate(.now, equalTo: forecast.date, toGranularity: .hour)
            return Hours
        } else {
            let Today = Calendar.current.isDate(.now, equalTo: forecast.date, toGranularity: .day)
            return Today
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 50)
                ...
                .innerShadow(shape: RoundedRectangle(cornerRadius: 50), color: .white.opacity(0.25), lineWidth: 1, offsetX: 1, offsetY: 1, blur: 0, blendMode: .overlay)
